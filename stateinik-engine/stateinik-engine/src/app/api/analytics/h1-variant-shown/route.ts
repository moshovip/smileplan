...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const rl = await checkRateLimit(ip, 'analytics-h1-shown', 100, 60)
  ...
  try {
    await prisma.guideH1Stats.upsert({
      ...
      create: { guideId: body.guideId, variantIndex: body.variantIndex, showns: 1 },
      update: { showns: { increment: 1 } },
    })
  } catch {
